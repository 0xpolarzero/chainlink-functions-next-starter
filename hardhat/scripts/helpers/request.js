...
const request = async () => {
  ...
  const { lastBaseFeePerGas, maxPriorityFeePerGas } = await ethers.provider.getFeeData()
  ...
  console.log(
    `\nIf all ${gasLimit} callback gas is used, this request is estimated to cost ${ethers.utils.formatUnits(
      estimatedCostJuels,
      18
    )} LINK`
  )
